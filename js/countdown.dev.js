...
(function( $ ){
    ...
    $.fn.Apocalypse = function( options ) {
        ...
        return this.each(function() {
			...
			var apocalypse = getValuesByTimestamp(settings["timestamp"]);
			
			days = apocalypse[0];
			hours = apocalypse[1];
			minutes = apocalypse[2];
			seconds =apocalypse[3];
			...
			*/
		   $(current).prepend( field.join('<div class="separator">:</div>') );
		   ...
		   if( global_Days < 1 && global_Hour < 1 && global_Mins < 1 && global_Secs < 1){
			...
			//jQuery(current).prepend("Hell yeah!"+ days + ':'+hours +':'+ minutes +':'+ seconds );
